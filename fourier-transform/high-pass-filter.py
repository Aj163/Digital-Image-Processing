...
img_filter = np.ones(img.shape)
SIZE = 25
dim_x = img.shape[0]
dim_y = img.shape[1]
img_filter[dim_x//2 - SIZE : dim_x//2 + SIZE, dim_y//2 - SIZE : dim_y//2 + SIZE] = 0

fshift = np.fft.fftshift(f)
magnitude_spectrum = 20*np.log(np.abs(fshift))

filter_fshift = fshift * img_filter
edge_f = np.fft.ifftshift(filter_fshift)
edge_img = np.fft.ifft2(edge_f)
